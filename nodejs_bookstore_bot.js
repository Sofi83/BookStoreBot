...
const token = process.env.BOT_TOKEN || '8231548488:AAGmNQYgHqAIlsoQ-dPDT92U05XGW5WIw9c';
...
Спасибо за покупку! 🙏
  ...
  // Сначала отправляем текстовое сообщение пользователю с файлом
  console.log(`📤 Начинаем отправку файла пользователю ${userId}`);
  console.log(`📚 Книга: ${order.bookTitle}, Формат: ${order.format}`);
  console.log(`🔗 Ссылка на скачивание: ${downloadLink}`);
  console.log(`✅ Ссылка проверена: ${downloadLink.includes('dl=1') ? 'Правильно (dl=1)' : 'Проверьте ссылку'}`);
  
  bot.sendMessage(userId, notificationText, { parse_mode: 'Markdown' })
    .then(() => {
      console.log(`✅ Уведомление с файлом отправлено пользователю ${userId}`);
