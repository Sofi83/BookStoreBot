// ============================================
// TELEGRAM BOT FOR BOOKSTORE
// Version: 1.1.0 - Improved error handling
// ============================================

// Загружаем переменные окружения из .env файла (для локальной разработки)
// На Railway переменные окружения устанавливаются через панель управления
try {
...
// Настройка бота: используем polling (для Railway можно также использовать webhook)
...
Переходите в каталог и выбирайте книгу 👇🏻`;
  ...
  const keyboard = {
    inline_keyboard: [
      [{ text: '📖 Показать каталог', callback_data: 'show_catalog' }]
    ]
  };
...
function showPaymentInfo(chatId, bookId, format) {
  ...
  // Получаем реквизиты из переменных окружения или используем дефолтные
  const cardNumber = process.env.CARD_NUMBER || '2202 2063 4567 8901';
  const cardHolder = process.env.CARD_HOLDER || 'Иванов Иван Иванович';
...
🔍 Проверяем  оплату....
⏰ Обычно это занимает до10 минут.
